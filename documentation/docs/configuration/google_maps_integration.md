...
To enable Google Maps integration in Global Router, you'll need to create a Google Maps API key. This key allows Global Router to use Google Maps services such as geocoding and location search throughout the application.
...
4. Click **New Project**.
5. Name your project (e.g., `Global Router Maps`) and click **Create**.
...
2. Under **Application restrictions**, choose one:
   ...
   - Add the domain of the Global Router **backend** (e.g., `https://your-globalrouter-backend.com`). -->

## Configuration in Global Router

Set the API key in your environment file or configuration under the backend service of Global Router. This is typically done in the `docker-compose.yml` file or directly in your environment variables `.env` file.

```env
GOOGLE_MAPS_API_KEY=your_api_key_here
```

Once this is set, Global Router will be able to utilize Google Maps services for geocoding and location searches instead of relying on the default OpenStreetMap services.
