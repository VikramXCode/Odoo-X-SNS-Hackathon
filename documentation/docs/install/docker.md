...
Docker is the preferred way to run Global Router on your local machine. It is a lightweight containerization technology that allows you to run applications in isolated environments called containers.
...
Get the `docker-compose.yml` and `.env.example` files from the Global Router repository. You can download them here:

- [Docker Compose](https://github.com/seanmorley15/Global Router/blob/main/docker-compose.yml)
- [Environment Variables](https://github.com/seanmorley15/Global Router/blob/main/.env.example)

```bash
wget https://raw.githubusercontent.com/seanmorley15/Global Router/main/docker-compose.yml
wget https://raw.githubusercontent.com/seanmorley15/Global Router/main/.env.example
cp .env.example .env
```

::: tip

If running on an ARM based machine, you will need to use a different PostGIS Image. It is recommended to use the `imresamu/postgis:15-3.3-alpine3.21` image or a custom version found [here](https://hub.docker.com/r/imresamu/postgis/tags). The Global Router containers are ARM compatible.

:::

## Configuration

The `.env` file contains all the configuration settings for your Global Router instance. Here’s a breakdown of each section:
...
Once you've configured `.env`, you can start Global Router with:

```bash
docker compose up -d
```

Enjoy using Global Router! 🎉
