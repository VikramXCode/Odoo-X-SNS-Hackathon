...
services:
  caddy:
    ...
    container_name: globalrouter-caddy
...
Since all ingress traffic to the Global Router containsers now travels through Caddy, we can also remove the external ports configuration from those containsers in the `docker-compose.yml`. Just delete this configuration:
...
Your Global Router should now be up and running.
