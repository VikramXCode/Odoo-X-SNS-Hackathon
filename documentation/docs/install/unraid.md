...
Global Router is available in the Unraid Community Applications store. You can install it by searching for `Global Router` in the Community Applications store, where you will find the frontend and the backend. The database can be found by searching `PostGIS`.

Community Applications Page for Global Router: [Global Router on CA Store](https://unraid.net/community/apps?q=Global Router)\
...
- There is **no** Global Router---Database app, to find the database application search for `PostGIS` on the Unraid App Store then add and fill out the fields as shown below
...
Youtuber AlienTech42 has created a helpful video walking through the installation of Global Router on Unraid:
