...
# =============================================================================
# Global Router Installer Script
# (c) 2026 Global Router Team
# License: GPL-3.0
# =============================================================================

APP_NAME="Global Router"
INSTALL_DIR="./globalrouter"
# COMPOSE_FILE_URL="https://raw.githubusercontent.com/seanmorley15/Global Router/main/docker-compose.yml"
# ENV_FILE_URL="https://raw.githubusercontent.com/seanmorley15/Global Router/main/.env.example"
...
╔═════════════════════════════════════════════════════════════════════════╗
║                                                                         ║
║             G L O B A L   R O U T E R   I N S T A L L E R               ║
║                                                                         ║
...
# Check for Global Router running as a docker container
check_running_container() {
    if docker ps -a --filter "name=globalrouter" --format '{{.Names}}' | grep -q "globalrouter"; then
        log_error "Global Router is already running as a Docker container (including stopped or restarting states)."
        echo ""
        echo "Running this installer further can break existing installs."
        echo "Please stop and remove the existing Global Router container manually before proceeding."
...
prompt_configuration() {
    ...
    echo ""
    echo "Configure the URLs where Global Router will be accessible."
...
POSTGRES_DB=globalrouter
POSTGRES_USER=globalrouter
...
EOF
    ...
    echo -e "   📦 Media Vol:  ${YELLOW}globalrouter_media${NC}"
    ...
    echo -e "${BOLD}❤️  Enjoying Global Router?${NC}"
    echo -e "   Support future development: ${MAGENTA}https://seanmorley.com/sponsor${NC}"
    echo ""

    echo -e "${BOLD}🌍 Adventure awaits — your journey starts now with Global Router!${NC}"
}
...
print_failure_message() {
    ...
    echo ""
    echo "For support, visit: https://github.com/seanmorley15/Global Router"
}
